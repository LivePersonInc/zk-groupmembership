...
class ZKCluster extends EventEmitter {
    /**
     *
     * @param {Object} conf
     ...
     * array that would be returned by getNodes(). Errors to retrieve node lists upon change will emit an 'error' event
     * @return {ZKCluster} this
     */
    startMonitor() {
        if(!this._clusterChildListener) {
            this._clusterChildListener = () => {
                this.getNodes().then(nodeList => this.emit('node-list', nodeList), err => this.emit('error', err));
            };
            this._clusterChildListener();
        }
        return this;
    ...
    _onState(state) {
        if (state !== Zookeeper.State.SYNC_CONNECTED) {
            this.emit('error', new Error('Connection Not Established'));
        }
    }
     ...
     * @throws {Error} If the instance id contains a '/'
     */
